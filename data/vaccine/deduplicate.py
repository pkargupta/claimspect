...
def main():
    ...
    # Add the arguments
    parser.add_argument('dataset', type=str, choices=['vaccine', 'dtra'], help='The name of the dataset', default='vaccine')
    ...
    with open(f'data/{dataset_name}/deduplicated_segments.json', 'w') as file:
        json.dump(deduplicated_data, file, indent=4)

    # Print the report sorted by id
    for id in sorted(report.keys(), key=int):
        stats = report[id]
